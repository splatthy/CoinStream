...
def show_confluence_analysis_page() -> None:
    ...
    try:
        ...
        if not trades:
            st.info(
                "No trade data available. Please configure exchanges and import data first."
            )
            return
    ...
    try:
        ...
        with col1:
            ...
            sort_by = st.selectbox(
                "Sort by:", options=list(sort_options.keys()), key="confluence_sort"
            )
            ...
            for i, metrics in enumerate(sorted_metrics[:5], 1):
                with st.container():
                    ...
                    st.caption(
                        f"Win Rate: {metrics.win_rate}% | PnL: ${float(metrics.total_pnl):,.2f}"
                    )
...
def render_confluence_summary(
    analysis_service: AnalysisService,
    trades: List[Trade],
    confluence_metrics: List[ConfluenceMetrics],
) -> None:
    ...
    try:
        ...
        with col1:
            st.metric("Total Confluences", confluence_stats["total_confluences"])
        ...
        with col4:
            ...
            st.metric("Avg per Trade", f"{avg_confluences:.1f}")

        with col5:
            multiple_pct = confluence_stats["multiple_confluence_percentage"]
            st.metric("Multiple Confluences", f"{multiple_pct:.1f}%")

    except Exception as e:
        logger.error(f"Error rendering confluence summary: {e}")


def render_confluence_performance_chart(
    metrics: List[ConfluenceMetrics], sort_by: str
) -> None:
        ...
        fig = go.Figure(
            data=[
                go.Bar(
                    ...
                    marker_color=colors,
                    hovertemplate=(
                        f"<b>%{{x}}</b><br>{y_label}: %{{y}}<br><extra></extra>"
                    ),
                )
            ]
        )
...
def render_confluence_comparison(
    analysis_service: AnalysisService, trades: List[Trade]
) -> None:
    ...
    try:
        ...
        if confluence1 and confluence2:
            # Perform comparison
            comparison = analysis_service.compare_confluences(
                trades, confluence1, confluence2
            )
            ...
            render_side_by_side_comparison(
                analysis_service, trades, confluence1, confluence2
            )
...
def render_side_by_side_comparison(
    ...
    try:
        ...
        render_statistical_significance_testing(
            conf1_trades, conf2_trades, confluence1, confluence2
        )

        # PnL distribution comparison
        render_pnl_distribution_comparison(
            conf1_trades, conf2_trades, confluence1, confluence2
        )

        # Performance over time comparison
        render_performance_over_time_comparison(
            conf1_trades, conf2_trades, confluence1, confluence2
        )
...
def render_confluence_performance_pie(
    trades: List[Trade], confluence_name: str
) -> None:
...
def render_pnl_distribution_comparison(
    trades1: List[Trade], trades2: List[Trade], conf1: str, conf2: str
) -> None:
    ...
    try:
        ...
        if pnl1:
            fig.add_trace(
                go.Histogram(
                    x=pnl1, name=conf1, opacity=0.7, nbinsx=20, marker_color="blue"
                )
            )
        ...
        if pnl2:
            fig.add_trace(
                go.Histogram(
                    x=pnl2, name=conf2, opacity=0.7, nbinsx=20, marker_color="orange"
                )
            )
...
def render_comparison_filters(
    analysis_service: AnalysisService, trades: List[Trade], confluence_list: List[str]
) -> None:
    ...
    try:
        ...
        with col2:
            ...
            selected_type = st.selectbox(
                "Trade Type:", options=trade_types, key="filter_trade_type"
            )
...
def render_filtered_analysis(
    ...
    try:
        ...
        top_metrics = sorted(
            confluence_metrics, key=lambda x: x.total_pnl, reverse=True
        )[:3]

        for i, metrics in enumerate(top_metrics, 1):
            st.markdown(
                f"{i}. **{metrics.confluence}** - Win Rate: {metrics.win_rate}%, PnL: ${float(metrics.total_pnl):,.2f}"
            )
...
def render_confluence_export_options(
    df: pd.DataFrame, metrics: List[ConfluenceMetrics]
) -> None:
...
def render_statistical_significance_testing(
    trades1: List[Trade], trades2: List[Trade], conf1: str, conf2: str
) -> None:
    ...
    try:
        ...
        with col3:
            ...
            pooled_std = np.sqrt(
                (
                    (len(pnl1) - 1) * np.var(pnl1, ddof=1)
                    + (len(pnl2) - 1) * np.var(pnl2, ddof=1)
                )
                / (len(pnl1) + len(pnl2) - 2)
            )
            cohens_d = (
                (np.mean(pnl1) - np.mean(pnl2)) / pooled_std if pooled_std > 0 else 0
            )
        ...
        with col4:
            ...
            se_diff = np.sqrt(
                np.var(pnl1, ddof=1) / len(pnl1) + np.var(pnl2, ddof=1) / len(pnl2)
            )
...
def render_performance_over_time_comparison(
    trades1: List[Trade], trades2: List[Trade], conf1: str, conf2: str
) -> None:
...
def generate_confluence_summary_report(metrics: List[ConfluenceMetrics]) -> str:
    ...
    try:
        ...
        report_lines.extend(["", "HIGHEST WIN RATES:", "-" * 20])
